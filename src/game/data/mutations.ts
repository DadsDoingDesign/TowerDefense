...
interface MutTemplate { key: string; name: string; desc: string; downside: string; mods: EffectMods }
...
const MUTATIONS: MutTemplate[] = [
  ...
  { key: 'rapid', name: 'Rapid Fire', desc: 'Doubles attack rate — each hit lands much lighter.', downside: '−45% damage per hit', mods: { rateMult: 2.0, damageMult: 0.55 } },
  { key: 'heavy', name: 'Heavy Ordnance', desc: 'Devastating 2.2× hits — at half the fire rate.', downside: '−50% attack speed', mods: { damageMult: 2.2, rateMult: 0.5 } },
  ...
  { key: 'executioner', name: 'Executioner', desc: 'Executes enemies below 28% HP; +12% crit — but strikes slower.', downside: '−15% attack speed', mods: { execute: 0.28, critChanceAdd: 0.12, rateMult: 0.85 } },
...
export function rollMutation(rng: RNG, exclude: string[] = []): Mutation {
  ...
  return { id: nextId('mut'), key: t.key, name: t.name, desc: t.desc, rarity: 'mythic', downside: t.downside, mods: t.mods }
}
